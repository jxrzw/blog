...
def wirte_article(request):
    if request.method == 'GET':
        ...
        if aform.is_valid():
            ...
            article.user = data.get('user')  #1对多　直接赋值
            article.save()
            article.tags.set(data.get('tags'))  #多对多必须在文章保存的后面在添加
...
def article_lmessage(request):
    ...
    if request.method == 'GET':
        ...
        if name and lytext:
            ...
            if message:
                return redirect(reverse('article:lmessage'))
